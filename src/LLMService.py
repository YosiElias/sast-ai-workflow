...
class LLMService:
    ...
    def critique_llm(self):
        if self._critique_llm is None:
            # Decide which LLM to use based on the base_url
            if "nvidia" in self._critique_base_url.lower():    # Todo delete before submit!!!
    ...
    def filter_known_error(self, database, user_input):
        ...
        answer_template = read_answer_template_file(os.path.join(os.path.dirname(__file__), "templates", "known_issue_resp.json"))
        chain1 = (
                {
